...
usertrap(void)
{
  ...
  /* give up the CPU if the alarm timer is due */
  if(which_dev == 2) {
    if (p->alarm_interval == 0 && p->tickpassed == 0) {
      yield();
    }
    else if (p->tickpassed < p->alarm_interval){
      p->tickpassed++;
    }
    else {
      p->tickpassed = 0;
      if (p->handler_returned) {
        saveregs(p);
